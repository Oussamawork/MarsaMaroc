                ...
                @foreach($materiels as $mat)
                    <tr>
                        ...
                        <td style="text-align:  center;">
                            {{ $mat->isReformed() ? 7 : ($mat->isDisponible() ? 5 : ($mat->isAffected() ? 5 : 4) )}}
                        </td>
                        <td style="text-align:center;">
                            <button class="m-portlet__nav-link btn m-btn m-btn--hover-success m-btn--icon m-btn--icon-only m-btn--pill btn-edit" data-id="{{ $mat->serial }}" onclick="myFunction(this);" title="Affecter" {{ $mat->isReformed() ? 'Disabled' : ($mat->isDisponible() ? '' : ($mat->isAffected() ? '' : 'Disabled')) }}>
                                <i class="flaticon-reply"></i>
                            </button>
                            
                            <a href="{{route('MatDesaffect',['id'=>$mat->id])}}" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Désaffecter" {{ $mat->isReformed() ? 'isDisabled' : ($mat->isDisponible() ? 'isDisabled' : ($mat->isAffected() ? 'isDisabled' : ''))}}>
                        ...
                        <td style="text-align:center;">
                            <span style="overflow: visible; position: relative; width: 110px;">
                                ...
                                <button class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill btn-edit"  data-toggle="modal" data-target="#Panne" data-id="{{ $mat->id }}" title="En Panne" {{ $mat->isReformed() ? 'Disabled' : ($mat->isDisponible() ? '' : ($mat->isAffected() ? '' : 'Disabled'))}}>
                                    <i class="flaticon-danger"></i>
                                </button>
                                
                                <button class="m-portlet__nav-link btn m-btn m-btn--hover-primary m-btn--icon m-btn--icon-only m-btn--pill" data-toggle="modal" data-target="#Reforme" data-id="{{ $mat->id }}" data-serial="{{ $mat->serial }}" title="Reforme" {{ $mat->isReformed() ? 'Disabled' : ($mat->isDisponible() ? '' : ($mat->isAffected() ? '' : 'Disabled'))}} >
                                ...
                                <a href="{{route('deleteMaterial',['id'=>$mat->id])}}" class="m-portlet__nav-link btn m-btn   m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill {{ $mat->isReformed() ? '' : ($mat->isDisponible() ? '' : ($mat->isAffected() ? '' : 'isDisabled'))}}" title="Delete">
